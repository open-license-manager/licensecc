...
static void set_preferred_disks(std::vector<DiskInfo> &diskInfos, std::unordered_map<std::string, int> &disk_by_uuid) {
	// On macOS, use getfsstat to get mounted filesystems
	struct statfs *mounts;
	int num_mounts = getfsstat(NULL, 0, MNT_NOWAIT);
	if (num_mounts < 0) {
		LOG_DEBUG("getfsstat failed");
		return;
	}

	mounts = (struct statfs *)malloc(num_mounts * sizeof(struct statfs));
	num_mounts = getfsstat(mounts, num_mounts * sizeof(struct statfs), MNT_NOWAIT);

	for (int i = 0; i < num_mounts; i++) {
		std::string device_name_s(mounts[i].f_mntfromname);
		LOG_DEBUG("found mount entry %s ", mounts[i].f_mntfromname);
		...
		for (auto &disk_info : diskInfos) {
			if (device_name_s.find(disk_info.device) != std::string::npos ||
			    std::string(disk_info.device).find(device_name_s) != std::string::npos) {
